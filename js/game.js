...
window.onload = function () {
    ...
    var setMaps = function () {
        ...
        map.loadData(mapData[BASE_FLOOR]);
        foregroundMap.image = game.assets['sprites.png'];
        foregroundMap.loadData(foregroundData[BASE_FLOOR]);
        var collisionData = [];
        for (var i = 0; i < foregroundData[BASE_FLOOR].length; i++) {
            collisionData.push([]);
            for (var j = 0; j < foregroundData[BASE_FLOOR][0].length; j++) {
                var collision = foregroundData[BASE_FLOOR][i][j] % 13 > 1 ? 1 : 0;
    ...
    var setStage = function () {
        ...
        //TODO REMOVE THIS!!!! ITS JUST HERE TO MAKE THE D_PAD SHOW UP ON MAIN SCREEN!!
        var junk = new enchant.ui.Button("", "light");
        game.rootScene.addChild(junk);
        //END TODO
    ...
    player.facing = function () {
        ...
        if (!facingSquare) {
            ...
            return foregroundData[BASE_FLOOR][facingSquare.y][facingSquare.x];
        }
    }
